...
class RTTLoopHelper(LoopHelper):
    """
    RTT version of LoopHelper that produces a summary report of chunks read and runtime
    """
    ...
    @rtt.wrappers.rtt_test(multiple_files=True)
    def run(self):
        self.prepare_jobs()     # split files for each job, prepare relevants inputs (scale1fb, isData, etc)
        ...
        self.submit_jobs()      # actually submit the jobs (local, Dask, condor) 
        elapsed_time = time.time() - start
        print("[RTTLoopHelper] Total time to run %d jobs on %d cores: %.2f minutes" % (len(self.jobs_manager), self.nCores, elapsed_time/60.)) 

        start = time.time()
        self.merge_outputs()    # merge individual pkl files into a single master pkl
        self.clean_up()
        elapsed_time = time.time() - start
        print("[RTTLoopHelper] Total time to merge %d outputs: %.2f minutes" % (len(self.outputs), elapsed_time/60.))
    ...
    def prepare_jobs(self):
        ...
        for sample, info in self.samples_dict.items():
            ...
            if self.debug > 0:
                print("[RTTLoopHelper] Running over sample: %s" % sample)
                print("[RTTLoopHelper] details: ", info)
            ...
            for year, year_info in info.items():
                ...
                if year not in self.years:
                    continue
                for path in year_info["paths"]:
                    files.append("root://{0}//{1}".format(self.server, path))
                ...
                for file_split in file_splits:
                    ...
                    if self.fast:
                        if job_id > 1:
                            if self.debug > 0:
                                print("[RTTLoopHelper] --fast option selected, only looping over 1 file for sample: %s (%s)" % (sample, year))
                            break
                    output = self.output_dir + self.selections + "_" + self.output_tag + "_" + sample + "_" + year + "_" + str(job_id) + ".pkl"
                    self.jobs_manager.append({
                        "info" : job_info,
    ...
    def submit_jobs(self):
        if self.batch == "local":
            if self.debug > 0:
                print("[RTTLoopHelper] Submitting %d jobs locally on %d cores" % (len(self.jobs_manager), self.nCores))
            ...
            for job in self.jobs_manager:
                if self.dry_run:
                    continue
                print(job)
                running_procs.append(multiprocessing.Process(target = self.loop_sample, args = (job, queue)))
                running_procs[-1].start()
    ...
    def loop_sample(self, job, queue):
        info = job["info"]
        sample = info["sample"]
        files = job["files"]
        output = job["output"]
        ...
        if self.debug > 0:
            print("[RTTLoopHelper] Running job with parameters", job)
        ...
        for file in files:
            if self.debug > 0:
                print("[RTTLoopHelper] Loading file %s" % file)
    ...
    def load_file(self, file, metadata, tree_name = "Events"):
        ...
        with uproot.open(file, xrootd_handler=rtt.objects.RTTSource) as f:
            if not f:
            ...
            # library = "ak" to load arrays as awkward arrays for best performance
            # how = "zip" allows us to access arrays as records, e.g. events.Photon
            self.reports.append(f._file._source.report)
        return events, photons
